  ...
  constructor(edge: Edge, source?: EdgeConfig) {
    ...
    if (Object.keys(this.components).length != 0 && Object.keys(this.factories).length == 0) {
      ...
      for (const componentId in this.components) {
        ...
        if (!factory) {
          // Only log if in development mode - missing factories for custom/new components are common
          if (!component.isEnabled) {
            // Skip warning for disabled components
            continue;
          }
          console.info("Factory definition [" + component.factoryId + "] for [" + componentId + "] is missing. This is normal for custom components.");
          continue;
        }
